...
import { formatDate, formatMoney, layout, p, rows, toPlainText, ul } from './templates'
...
export async function sendDailyDigestEmail(params: {
  ...
  tasks: string[]
  staleLeads: string[]
  showingsToday: string[]
}): Promise<EmailSendResult> {
  const ctx = await getOrgEmailContext(params.orgId)
  const section = (heading: string, items: string[]) =>
    items.length ? `<h2 style="margin:20px 0 8px;font-size:15px;color:#232A24;">${heading}</h2>${ul(items)}` : ''

  const html = layout(
    'Дайджест на сегодня',
    section(`Оплаты (${params.overdue.length})`, params.overdue) +
      section(`Задачи (${params.tasks.length})`, params.tasks) +
      section(`Показы сегодня (${params.showingsToday.length})`, params.showingsToday) +
      section(`Лиды без движения (${params.staleLeads.length})`, params.staleLeads),
    {
      companyName: ctx.companyName,
      footerNote: ctx.footerNote,
      cta: { label: 'Открыть CRM', url: `${getSiteUrl()}/dashboard` },
    }
  )
  ...
  return sendEmail({
    orgId: params.orgId,
    to: params.to,
    subject: 'Дайджест HousePro на сегодня',
    html,
    kind: 'daily_digest',
    companyName: ctx.companyName,
  })
