...
import { ArrowDownCircle, ArrowUpCircle, Wallet, Tag, CircleAlert } from 'lucide-react'
...
function sectionTitle(icon: React.ReactNode, text: string) {
 return (
 <h2 className="hp-h2 flex items-center gap-2 mb-4">
...
export function TransactionForm({ transaction, categories, contracts, deals, employees, contacts, properties, managedPropertyIds = [], defaultPropertyId, defaultContractId }: Props) {
 ...
 style={{
 borderColor: type === t ? (t === 'income' ? 'var(--hp-accent)' : 'var(--hp-danger)') : 'var(--hp-border)',
 ...
 <div className="space-y-1.5">
 <label className="hp-label">Дата операции *</label>
 <input
 type="date"
 name="date"
 ...
 <div className="space-y-1.5">
 <label className="hp-label">Способ оплаты</label>
 <select
 name="payment_method"
 ...
 <div className="space-y-1.5">
 <label className="hp-label">Срок оплаты</label>
 <input
 type="date"
 name="due_date"
 ...
 <p className="text-xs text-[var(--hp-sub)]">Привязки — необязательно: свяжите операцию с объектом, договором, сделкой, контактом или сотрудником</p>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div className="space-y-1.5">
 <label className="hp-label">Объект</label>
 <select
 name="property_id"
 value={propertyId}
 ...
 <div className="space-y-1.5">
 <label className="hp-label">Договор</label>
 <select
 name="contract_id"
 ...
 {c.contract_number ? `№${c.contract_number}` : c.id.slice(0, 8)} · {CONTRACT_TYPE_LABELS[c.contract_type] ?? c.contract_type}
 </option>
 ))}
 </select>
 </div>
 ...
 <div className="space-y-1.5">
 <label className="hp-label">Контакт (контрагент)</label>
 <select
 name="contact_id"
 ...
 <div className="space-y-1.5">
 <label className="hp-label">Сотрудник (для зарплат)</label>
 <select
 name="employee_id"
 ...
 <button type="submit" disabled={isPending} className="hp-btn-primary disabled:opacity-60">
 {isPending ? 'Сохранение…' : (isEdit ? 'Сохранить' : 'Создать операцию')}
 </button>
 <Link href="/accounting" className="hp-btn-secondary">Отмена</Link>
 </div>
 </form>
 )
}
