...
export function AvitoSettingsForm({ clientId, maskedSecret, avitoUserId, contactPhone, isEnabled }: Props) {
  ...
  return (
    <form action={formAction} className="bg-white rounded-[20px] border border-slate-100 p-5 space-y-4" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.06)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            className="w-full h-10 px-4 rounded-xl border border-input bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all font-mono"
          />
      ...
      <label className="flex items-center gap-2.5 cursor-pointer w-fit">
        <input type="checkbox" name="is_enabled" defaultChecked={isEnabled} className="w-4 h-4 rounded border-input accent-[#16A34A]" />
      ...
      <div className="flex items-center gap-3 pt-1">
        <button
          type="submit"
          disabled={isPending}
          className="flex items-center gap-2 px-5 py-2.5 text-white rounded-[14px] text-sm font-bold hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:hover:translate-y-0"
