...
export function AvitoPublishToggle({ propertyId, isPublished, status, error, syncedAt, eligible }: Props) {
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-[20px] shadow-sm p-5 space-y-3">
      ...
      {isPublished && error && (
        <p className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-lg p-2 leading-relaxed break-words">
          {error}
        </p>
      )}
      ...
      {!eligible && !isPublished ? (
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-[14px] text-sm font-bold transition-all hover:-translate-y-0.5 disabled:opacity-60 disabled:hover:translate-y-0"
          style={
            isPublished
