...
export function LeadActivityForm({ leadId }: { leadId: string }) {
 ...
 className="w-full flex items-center justify-center gap-2 py-2 border border-dashed border-border text-sm text-muted-foreground hover:border-primary hover:text-primary transition-all"
 >
 ...
 return (
 <form action={handleSubmit} className="bg-muted/30 border border-border p-4 space-y-3">
 {/* Тип */}
 <div className="flex flex-wrap gap-1.5">
 {activityTypes.map(t => (
 <label key={t.value}
 className="flex items-center gap-1.5 px-2.5 py-1.5 border border-border cursor-pointer text-xs hover:bg-accent transition has-[:checked]:border-primary has-[:checked]:bg-primary/10 has-[:checked]:text-primary">
 ...
 className="px-4 h-10 border border-border text-sm font-medium hover:bg-accent transition-all">
 Отмена
 </button>
 </div>
 </form>
