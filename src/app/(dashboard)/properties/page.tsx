...
const statusColors: Record<string, string> = {
  ...
  rented: 'bg-blue-100 text-blue-700', sold: 'bg-gray-100 text-gray-600',
      ...
      <div className="bg-card border border-border rounded-2xl p-4 flex flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link href="/properties"
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${!params.deal_type ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
            Все
          </Link>
          {Object.entries(dealLabels).map(([value, label]) => (
            <Link key={value} href={`/properties?deal_type=${value}`}
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${params.deal_type === value ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
              {label}
            </Link>
          ))}
        </div>
      ...
      {!properties || properties.length === 0 ? (
        <div className="bg-card border border-border rounded-2xl text-center py-16">
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {error ? `Ошибка: ${error.message}` : 'Добавьте первый объект'}
          </p>
          <Link href="/properties/new"
            ...
            <Link key={property.id} href={`/properties/${property.id}`}
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
                <div>
                  ...
                  {property.area && (
                    <p className="text-xs text-muted-foreground">{property.area} м²
