...
export default async function NewContractPage({
  ...
  const [{ data: clients }, { data: properties }] = await Promise.all([
    ...
    supabase.from('properties').select('id, title, address').eq('status', 'available').order('title'),
  ])
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Создайте договор с клиентом</p>
        </div>
      </div>
