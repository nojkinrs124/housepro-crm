...
export default async function ExportPage() {
 ...
 {platforms.map(p => (
 <div key={p.name} className="bg-white border border-slate-100 p-6 space-y-4" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.06)' }}>
 <div className="flex items-center gap-3">
 <div className={`w-11 h-11 flex items-center justify-center shrink-0 ${p.iconBg}`}>
 ...
 className="flex-1 flex items-center justify-center gap-2 py-2.5 text-white text-sm font-bold transition-all hover:-translate-y-0.5"
 style={{ background: p.btnBg, boxShadow: p.btnShadow }}>
 <Download style={{ width: 16, height: 16 }} />
 Скачать XML
 </a>
 <a href={p.docsUrl} target="_blank" rel="noopener noreferrer"
 className="flex items-center justify-center px-4 py-2.5 border border-slate-200 text-sm font-semibold text-muted-foreground hover:bg-slate-50 hover:text-foreground transition-all shrink-0">
 Docs
 </a>
 </div>
 </div>
 ...
 {/* CSV / 1C export */}
 <div className="bg-white border border-slate-100 p-6 space-y-4" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.06)' }}>
 <div className="flex items-center gap-3">
 ...
 key={item.url}
 href={item.url}
 className="flex items-center justify-center gap-1.5 py-2.5 px-3 border border-slate-200 text-sm font-medium text-slate-600 hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700 transition-colors"
 >
