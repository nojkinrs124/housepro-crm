...
function buildFaq(legalForm: keyof typeof LEGAL_FORM_COPY): WhatIfItem[] {
  ...
  if (legalForm !== 'ooo') {
    items.push({
      ...
      a: `Форма — это про налоги, а не про ответственность. Договор с ${form.instrumental} имеет ту же силу, что и с ООО, а отвечает по нему конкретный человек с именем и ИНН, а не компания с уставным капиталом в десять тысяч. Реквизиты — ниже на этой странице и в каждом договоре.`,
    })
  }
  ...
  items.push(
    {
      ...
      a: `Договор найма заключён между вами и нанимателем — он не зависит от агентства и продолжает действовать. Договор с агентством расторгается по уведомлению за ${USLUGI.agencyTermination.noticeDays} дней: за это время мы передаём вам документы, ключи, депозит и контакты нанимателя по акту. Квартира не останется без хозяина ни на день.`,
    },
    {
