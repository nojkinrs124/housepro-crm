...
> Source Serif 4 + Source Sans 3, плоские панели без теней) и **1b
...
> переведены на реестры и блоки полностью — теперь это эталон вместе с `contacts`.
>
...
'var(--hp-radius)'        // 0 — карточки, кнопки, инпуты, иконки-боксы, панели
'var(--hp-radius-sm)'     // 0 — вложенное: строки списка, чипы, плитки внутри карточки
'var(--hp-radius-badge)'  // 0 — бейджи: прямоугольная метка, не pill
...
`.hp-block` даёт плоскую панель (как и все контейнеры в «Кабинете» — без скругления);
...
`bg-violet-100` для договоров и т.п.) **полностью вычищено 01.09.2026**: в `src/`
не осталось ни одного класса вида `bg-*-500` / `text-*-600` / `border-*-200` из
палитры Tailwind — только токены `var(--hp-*)`. Единственный акцент в системе —
`var(--hp-accent)`, цвет несёт только семантику статуса (good/warn/danger/info),
не принадлежность к модулю. Иконка-бокс в стат-карточке нейтральный: цветным он
становится, только если показатель действительно тревожный (например «Просрочено»).
...
| Компонент | Где | Зачем |
|---|---|---|
| `PageHeader` | `components/layout/` | Крошки + H1 + бейджи статуса + мета-строка + кнопки действий. Все шапки страниц только через него. |
| `Breadcrumbs` | `components/layout/` | Капс-крошки `СДЕЛКИ / ПЕРЕГОВОРЫ / СД-2041`. Обычно передаются в `PageHeader` пропом `crumbs`. |
| `StatStrip` | `components/layout/` | Полоса показателей: одна панель, ячейки через линии. Заменяет ряд отдельных мини-карточек. |
| `RegistryToolbar` | `components/layout/` | Поиск + фильтр-чипы + переключатель вида над реестром. Презентационный — состояние остаётся в свитчере. |
| `DealFormBody` | `features/deals/components/` | Тело формы сделки, общее для `new` и `edit`. |

Форматтеры — в `src/lib/utils.ts`: `formatAmount` (сумма без ₽, знак ставится отдельным
приглушённым спаном), `formatDateCompact`, `formatRelative` («2 ч. назад»),
`formatDeadline` (`{ label, overdue }` для просрочек), `initials`.

Готовые классы «Кабинета» (все — в `globals.css`): `.hp-block` / `.hp-block-header` /
`.hp-block-row` / `.hp-block-grid` (в две колонки) / `.hp-block-item` (произвольная
строка в блоке), `.hp-strip*`, `.hp-registry` + `.hp-registry-foot` + `.hp-page-btn`,
`.hp-stages` / `.hp-stage`, `.hp-chip`, `.hp-crumbs`, `.hp-meta`, `.hp-avatar`.
...
- [ ] Нет захардкоженных `#16A34A`/`#22C55E`/`bg-*-50`/`text-*-600`/`bg-gradient-*`/`rounded-[Npx]`/`shadow-*` — только токены `var(--hp-*)`
- [ ] Пустое состояние — `hp-card hp-empty` + нейтральная иконка-бокс, а не заливка цветом во весь блок
