...
**`any` в проекте больше нет** — весь хвост от старой заглушки `Database = any` снят
04.09.2026 (208 → 0, задача #21), baseline переснят на нули. `check:any` теперь просто
не пропускает новые. Если доменный тип разошёлся со схемой — править тип, а не ставить
каст на месте: под каждым из снятых `any` в итоге нашлись настоящие баги.
...
Целевой пользователь — нанятый риелтор, впервые открывший CRM. Всё, что требует
объяснений, — дефект. Экран готов, только если проходит все восемь правил:

1. **≤ 3 кликов** от `/dashboard` до завершения частого действия.
2. **Одна главная кнопка на экран** — `hp-btn-primary`. Остальные вторичные или в меню «…».
3. **≤ 6 полей на виду** в форме, остальное свёрнуто в «Дополнительно». Обязательных — минимум.
4. **Единый скелет модуля:** список → фильтр → карточка → действия. Без уникальных структур.
5. **Нет пустых экранов:** что это и что делать + кнопка, которая это делает.
6. **Человеческий язык:** никаких `deal_stage`, `null`, `error.message` от Postgres. Ошибка говорит, что случилось и что делать.
7. **Необратимое подтверждается, обратимое — нет.** Удаление спрашивает и называет последствия; сохранение просто сохраняет.
8. **Одно действие — один способ.** Дубли кнопок и путей убираются.

Эталон — модуль **Сделки** (`src/app/(dashboard)/deals`, `src/features/deals`).
Новые экраны строятся только из этих компонентов:

| Компонент | Где | Для чего |
|---|---|---|
| `PageHeader` + `StatStrip` + `RegistryToolbar` + `RegistryTable` | `src/components/layout`, `src/features/registry` | скелет списка (уже везде) |
| `EmptyState` | `src/components/layout/EmptyState.tsx` | правило 5: нет данных / фильтр пуст |
| `RecordActions` + `OverflowMenu` | `src/components/layout` | правило 2: primary + secondary + «…» |
| `ConfirmDeleteButton` | `src/components/forms/ConfirmDeleteButton.tsx` | правило 7: `action={deleteXAction.bind(null, id)}`, текст с последствиями, ошибка — toast |
| `FormSection` · `FieldGrid` · `Field` · `FormExtra` · `FormActions` | `src/components/forms/FormLayout.tsx` | правила 3, 4: секции, подсказки, «Дополнительно», одна кнопка |
| `friendlyDbError()` | `src/lib/errors.ts` | правило 6: вместо `return { error: error.message }` |

Обратная связь: **toast** (sonner) — для действий на месте (смена статуса, удаление,
публикация); **баннер** `ServerActionForm` — для форм с редиректом. Не смешивать.

Скрытое, но не удалённое, записывается в `docs/HIDDEN.md` с датой и причиной.
...
| `stop-handbook` | напоминает перечитать главу `docs/handbook/`, если менялись экраны её раздела |
...
| Как работать в системе (для сотрудников) | `docs/handbook/` — справочник по разделам |
| Новая фича через спецификацию (Spec Kit) | `docs/SPEC_KIT.md` — цикл specify → clarify → plan → tasks → implement |
