...
test.describe('Новые разделы CRM', () => {
  ...
  test('предпросмотр графика платежей считается на клиенте', async ({ page }) => {
    ...
    //
    // Сумму депозита задаём здесь, а не полагаемся на залог из договора: тест
    // берёт ПЕРВЫЙ договор из списка, а у него залога может не быть вовсе —
    // например, у договора управления его не бывает. Раньше тест на это
    // опирался и падал, как только самым свежим договором оказался не наём.
    await depositToggle.check()
    await page.locator('#sch-deposit').fill('25000')
