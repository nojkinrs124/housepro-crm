...
| 👥 **Сотрудники** | Роли (admin/manager/agent/accountant), KPI-статистика. Заведение — пока через панель Supabase: приглашения по email ещё не подключены |
...
Прогоняет по цепочке: проверку типов → сканер event-хендлеров в Server Components → анализ границы client/server (импорты клиентских функций на сервере и функции-пропы, уходящие в клиентские компоненты) → визуальный стандарт → частоту кронов → сборку → тесты. Тот же скрипт выполняется в CI при каждом push/PR.
