...
**Колонки оставлены в базе.** `DROP COLUMN` заблокирован хуком `guard-migration`,
и обходить его ради пустых колонок незачем: они никому не мешают, а данных в них
нет. Вместо удаления на каждую повешен комментарий прямо в схеме — тот, кто
откроет таблицу через год, увидит, что колонка не используется, и не станет
гадать.
