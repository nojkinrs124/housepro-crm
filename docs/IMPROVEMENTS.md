...
| 17 | ~~Legacy `clients`/`owners` ещё используются~~ — код отвязан, таблицы можно дропать | ✅ Done | Средняя | Данные |
...
**Дропать таблицы или нет — решение Руслана.** Код не мешает, но `DROP TABLE`
блокируется хуком `guard-migration`; выполнять только с явного подтверждения.
