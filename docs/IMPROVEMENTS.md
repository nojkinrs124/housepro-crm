...
| 19 | Приглашение сотрудников не реализовано | 🟠 P1 | Средняя | Функциональность |
| 20 | ESLint не работает: typescript-eslint не поддерживает TS 7 | 🟠 P1 | Низкая | Качество |
| 21 | 208 `any` остались после генерации типов | 🟡 P2 | Высокая | Надёжность |
| 22 | E2E: `/search` падает с net::ERR_ABORTED | 🟡 P2 | Низкая | Тесты |
