...
### 21. 208 `any` остались после генерации типов

**Проблема:** типы схемы больше не `any`, но написанные раньше `as any` и подавления
линтера сами собой не исчезли — 208 штук в 50 файлах. Теперь почти все они не нужны:
под ними лежит точный тип.
...
| 21 | 208 `any` остались после генерации типов | 🟡 P2 | Высокая | Надёжность |
...
| 27 | Лимиты тарифов объявлены, но не применяются нигде | 🟡 P2 | Средняя | Биллинг |
...
- **#27 Лимиты тарифов не применяются.** `PLAN_LIMITS` описывает max_users /
  max_properties / max_contracts / api_access / webhooks, но проверка была только
  в `checkLimit`, которую никто не вызывал. Функцию удалили как мёртвую — сами
  лимиты остались только текстом на странице биллинга. Решить: применять при
  создании сущностей (осторожно: существующие организации могут быть уже за
  лимитом Free) или убрать колонки из тарифной таблицы.
