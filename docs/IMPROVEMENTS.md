...
**Ещё один #28 в боте.** У меню лидов была своя копия воронки со статусом
`meeting`, которого нет ни в базе, ни в вебе. Кнопка «следующий статус»
записала бы его в `leads.status`, после чего лид пропал бы с доски — ровно та
поломка, что разбиралась в #28. Не выстрелило только потому, что кнопку не
нажимали: статусов `meeting` в базе нет. Воронка и подписи теперь берутся из
`LEAD_STATUSES`; заодно заведены единые `LEAD_SOURCES` и `TASK_PRIORITIES`
вместо копий в боте и вебе.
