...
- **#35 бакет `files`** — публичный, лежат только картинки постов канала. При
  этом `src/features/files/actions/files.actions.ts` пишет в бакет `documents`,
  которого в проекте нет: раздел «Файлы» не работает вообще, а когда заработает —
  выложит вложения через `getPublicUrl`. Чинить вместе: приватный бакет с
  org-изоляцией и подписанные ссылки.
