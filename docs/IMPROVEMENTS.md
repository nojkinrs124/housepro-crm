...
| 31 | Открытая регистрация в Supabase Auth — учётку в проде заводит кто угодно | 🔴 Открыто | Низкая | Безопасность |
...
| 39 | Проверка утёкших паролей в Supabase Auth выключена | 🟡 Открыто | Низкая | Безопасность |
...
- **#31 открытая регистрация** — переключатель в Supabase Dashboard, из сессии
  недоступен. Пока включён, любой желающий заводит учётку в проде; членства в
  организации у него нет и данные CRM ему не видны, но до сегодняшней миграции
  этого хватало, чтобы скачать все договоры из хранилища.
