...
- **UI** — `GenerateListingButton` + `GenerateListingDialog` на общем
  `QuickCreateModal` (добавлен `size="lg"`). Секция «Объявление» на странице
  редактирования стоит **вне** формы объекта: `<dialog>` внутри `<form>` ловил
  бы Enter как submit. Сохранённый текст виден на странице объекта.
