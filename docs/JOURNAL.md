...
## Сессия 03.09.2026 — защита от дурака: порядок заведения перестал быть обязательным

Задача от Руслана: «не всегда всё по порядку удаётся заводить, надо как-то
обезопасить, чтобы при создании одного другое не падало».

**Найдено при разборе — реальная потеря данных.** Форма объекта отправляла
`owner_contact_id`, а `extractPropertyFields` в `properties.actions.ts` это поле
не читал: собственник объекта **не сохранялся никогда**, ни при создании, ни
при редактировании (поля собственника в форме редактирования не было вовсе).
Отсюда прочерк в разделе «Управление» и в отчёте собственнику. FK
`properties.owner_id → contacts` был починен ещё миграцией
`20260901_contact_dedupe_and_owner_fk`, сломан оставался только код. Ни `tsc`,
ни `next build` такого не ловят — FormData не типизирована, лишний ключ молча
теряется.

Сделано:

- **Собственник объекта сохраняется.** Поле добавлено в `extractPropertyFields`
  (`owner_contact_id` → колонка `owner_id`), поставлено на форму создания и
  редактирования, в карточке объекта появился блок «Собственник».
- **`src/lib/readiness.ts`** — правила «что молча не сработает из-за пустого
  поля» для объекта, контакта, лида, договора и операции учёта. Чистые функции
  без supabase и JSX, 28 юнит-тестов. Формулировка правила — не «поле пустое», а
  «объявление не уйдёт на площадки», «напоминание не отправится», «в договоре
  останется пропуск».
- **Блок «Чего не хватает»** (`ReadinessPanel`) в карточках объекта, контакта,
  лида, договора, операции и объекта в управлении. Красная точка — критично,
  жёлтая — можно работать. Ничего не запрещает.
- **Расхождение статуса и договора** — отдельное правило: договор аренды при
  статусе «Доступно» (сданный объект висит в рекламе) и статус «Сдано» без
  договора (арендатор не появится в «Управлении»).
- **Быстрое создание связанных карточек** из формы: собственник в объекте,
  контакт и объект в задаче, объект в показе. `OwnerSelectField` обобщён в
  `ContactSelectField` (пропсы `name`/`label`/`role`) и переехал в contacts.
- **Дубли по телефону** — проверка добавлена в модалку быстрого создания
  контакта и в форму лида (`PhoneDuplicateField`, ищет и по контактам, и по
  лидам). Предупреждает, не блокирует.
- **`scripts/checks/form-fields.mjs`** — постоянная защита от повторения того же
  бага: поле формы сверяется с её обработчиком (Server Action + ключи zod-схемы,
  с раскрытием компонентов формы вроде `ContactSelectField`). Сверять глобально
  по проекту бессмысленно — `owner_contact_id` есть в схеме договора, и потеря
  поля объекта так не видна. Встроено в `npm run check` и в hook `post-edit`;
  проверено на настоящем регрессе — ловит.

`checkPayment` из readiness убран сразу после написания: раздел `/payments` —
legacy-редирект на `/accounting`, живые начисления лежат в
`accounting_transactions`. Правило про срок оплаты переехало туда же.

`npm run check` зелёный целиком, 212 тестов.
