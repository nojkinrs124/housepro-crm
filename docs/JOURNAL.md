...
## Сессия 20.09.2026 — теги текстом в постах канала

Руслан прислал скриншоты: в черновике поста `<a href="…">` виден буквально, в
недельной сводке — `<b>`. Разбор: санитайзер Telegram-HTML (02.08) пропускал
только `<b>/<i>/<code>`, а CTA-ссылка `<a href>` появилась 05.09 — с этого дня
два опубликованных поста в канале (16.09 и 18.09) вышли с тегом текстом.
Сводка ломалась иначе: превью «лучшего поста» резалось на 80 символах с
незакрытым `<b>`, Telegram отвечал «can't parse entities», ретрай без
разметки показывал все теги.

Сделано: `<a href="http(s)://…">` разрешён в санитайзере; превью в сводке без
тегов; админ-команда `/repair_cta` перепечатывает опубликованные посты через
`editMessageCaption`/`editMessageText` (токен бота только на Vercel, из сессии
не запустить — жмёт Руслан). Команду убрать через релиз-два, когда посты
починены.

---

## Сессия 17.09.2026 (вечер) — Telegram-бот: «Занести в CRM» вместо угадывания

Руслан: через бота присылаю фото паспортов арендаторов и собственников, выписки
ЕГРН — бот путается, лагает, источники в CRM не те. Просил аудит и решение.

Аудит по коду и живой базе показал, что «путается» — не про модель, а про
архитектуру: тип документа и действие угадывала модель по общему промпту на 85
строк с 26 инструментами. Конкретно:

- фото без подписи уходило модели с текстом «это чек, найди сумму»;
- две страницы паспорта из альбома — два параллельных вызова модели, два
  подтверждения, и оба перетирали одну строку истории;
- история чата хранила base64 фото и PDF — 1,4 МБ на 20 сообщений — и целиком
  уходила модели на каждое «какие задачи?»: это и были лаги;
- `import_rental_contract` писал `deal_type='rent'`/`status='contract'`, которые
  CHECK не пускает с миграции воронок 03.09 — функция была сломана, но после
  24.08 её не вызывали;
- источники: `telegram_document` (нет в справочниках), `api`, NULL; лид без
  `contact_id`; дедуп по сырой строке телефона, по паспорту — нигде;
- подтверждение показывало имя и телефон, ошибку в паспорте заметить было негде;
- побочно: форма лида с утра предлагала «Сдать/Продать», а CHECK их не пускал.

Решение — тип задаёт пользователь, модель только вычитывает, Postgres связывает:

- меню **📥 Занести в CRM** → арендатор / собственник / объект / договор;
  материалы копятся в `bot_intake_sessions` (файлы — в приватном `documents`),
  модель вызывается **один раз** по «Готово» с промптом под тип и принудительным
  инструментом `submit_extraction`, без истории чата;
- карточка подтверждения показывает всё: паспорт, регистрацию, объект,
  сомнения модели и ровно то, что будет создано; поправки — текстом;
- одна функция `import_intake(kind, payload)`: дедуп по нормализованному телефону
  **или** серии+номеру, объект по кадастровому номеру, лид с `contact_id`,
  сделка с `lead_id`/контактами/объектом, `needs_review`, источник `telegram`
  у всех; старые три функции — обёртки над ней, публичный API не сломан;
- сканы переезжают в карточку (`files`: паспорт → контакт, выписка → объект,
  договор → сделка);
- фото без сессии — вопрос «что это?» кнопками, «🧾 Чек» уходит в прежний
  `add_transaction`;
- дедуп `update_id` (`bot_processed_updates`), история без бинарников,
  промпт диалога вдвое короче, блок про канал видит только владелец;
- убрано: раздел «Анализ рынка» (дубль инструмента), команды `/post /case
  /pause /resume` (дубль кнопок), семь инструментов модели (см. `HIDDEN.md`);
- CHECK `leads.deal_type` расширен на `let`/`sell`, источники бота бэкфилнуты
  в `telegram`.

Решение, принятое без Руслана: из паспорта арендатора создаётся контакт + лид
(сразу `converted`, чтобы не висеть на доске второй копией) + сделка «на
проверку»; кнопка «Только контакт и лид» — для тех, кто до сделки не дошёл.

---

## Сессия 15.09.2026 — фото объектов переехали в Yandex Object Storage

Руслан спросил, где лежат фото, и захотел «на Гугл или Яндекс диске». Разобрали:
«диски» не дают стабильной прямой ссылки на файл, а фиды площадок забирают
картинки именно по URL — поэтому вместо Яндекс.Диска взяли Yandex Object
Storage (S3), а Google Drive отвергли (нестабилен в РФ, сервисный аккаунт не
может владеть файлами в личном Drive).

Сделано:

- В консоли Яндекса через браузер: платёжный аккаунт (карту привязал Руслан
  сам), бакет `housepro-photos` (публичное чтение объектов), сервисный аккаунт
  `housepro-storage` со `storage.editor`, статический ключ `housepro-vercel`.
  Секрет ключа в чат и в env не вносился — только Руслан руками.
- **`src/lib/storage/photo-storage.ts`** — адаптер `uploadPhoto` /
  `removePhotoByUrl` с переключателем `STORAGE_DRIVER=yandex|supabase`
  (по умолчанию supabase, чтобы деплой без новых env не сломался). Экшены фото
  ходят через него; удаление ищет файл по форме URL, а не по драйверу.
- `@aws-sdk/client-s3` в зависимостях, `storage.yandexcloud.net` в
  `images.remotePatterns`.
- `scripts/migrate-photos-to-yandex.mjs` (`npm run migrate:photos`) — перенос
  существующих файлов с переписыванием `photo_urls`; на момент сессии бакет
  Supabase был пуст, переносить было нечего.
- 7 юнит-тестов на адаптер: выбор драйвера, ошибка без ключей, оба пути удаления.

Не сделано: логотипы компаний и «Файлы» остались в Supabase Storage —
осознанно, их не тянут площадки.

Вечером Руслан загрузил 19 реальных фото — дошло 4–5. Две причины, обе не
про Яндекс: (1) Server Action режет тело на 1 МБ — первые снимки падали с
«Body exceeded 1 MB limit» (поднят до 12 МБ в `next.config.ts`); (2) даже
после этого каждый файл шёл через Vercel в США ~8–10 с, а функция на Hobby
живёт 10 с — часть отваливалась по таймауту, остальные ждали в очереди по
одному, и это выглядело как зависание. Переделано на прямую загрузку из
браузера в бакет по подписанной ссылке (CORS на бакете), по три параллельно,
с прогрессом и предупреждением при уходе со страницы. Заодно — перетаскивание
плиток для порядка публикации (`reorderPropertyPhotosAction`), которого
не хватало.

Следом: «обновил объект в CRM, на сайте не обновилось». Карточка и каталог
сайта рендерятся на запрос, а главная — статическая с ISR 60 с и
stale-while-revalidate: правка доезжала со второго визита. Экшены объекта
сбрасывали только страницы CRM. Добавлен `revalidateSiteForProperty()`
(`src/features/site/lib/revalidate.ts`) — сброс `/`, `/catalog`,
`/catalog/[id]`, `/sitemap.xml` — во все пути, что пишут в `properties`:
CRUD и фото объекта, завершение сделки (статус), импорт, API v1.

---

## Сессия 15.09.2026 — «Сгенерировать объявление» в карточке объекта

Задача от Руслана: надиктованные заметки по квартире + поля карточки → готовый
текст для Авито, без выдумок со стороны модели.

Сделано:

- **Миграция** `20260915_properties_listing_text.sql` — пять колонок `listing_*`
  в `properties`; история генераций отдельной таблицей отложена (#41).
- **`src/lib/ai/listing/`** — `facts.ts` (какие поля объекта уходят в промпт,
  разбор ответа по `---`; общий для сервера и клиента) и `prompt.ts` (системный
  промпт константой + `buildUserPrompt`). Пустые поля в промпт не попадают вовсе,
  снятые чекбоксы — тоже (снятая галочка ≠ «нет»). `management_fee` намеренно
  не передаётся: это комиссия управления для собственника, модель выдавала бы
  её за комиссию арендатора.
- **`/api/ai/listing`** — POST со стримом: сессия + `requirePermission`, объект
  читается из БД с проверкой `organization_id`, лимит 10 генераций / 10 минут,
  SSE OpenRouter разбирается на сервере и клиенту уходит чистый текст. Обрыв
  клиента (закрыл модалку, «Стоп») через свой AbortController останавливает и
  запрос к модели — проверено по времени ответа в логах dev-сервера.
- **UI** — `GenerateListingButton` + `GenerateListingDialog` на общем
  `QuickCreateModal` (добавлен `size="lg"`). Кнопка стоит в блоке «Описание»
  формы объекта; «Вставить в описание» пишет заголовок + тело в `description`
  (это поле читают фиды Авито/ЦИАН/Домклик) и подставляет текст в textarea на
  экране без перезагрузки. Первая версия держала отдельный блок «Объявление» —
  Руслан на живом прогоне копировал текст в «Описание» руками, дубль убран.
- Общего OpenRouter-клиента в проекте не оказалось — каждый вызов делает `fetch`
  на месте (бот, посты канала, проверка договора). Стриминга нигде не было,
  переиспользовать нечего; роут написан по тому же паттерну.

Наблюдение с живого прогона: при противоречии CRM и вводных (залог 10 000 в
карточке против «залог как аренда» в заметках) модель берёт вводные. Так и
оставлено, правило вписано в промпт явно — заметки свежее карточки.

---

## Сессия 03.09.2026 (продолжение) — «Оформить сделку» одной кнопкой

Задача от Руслана: «пришёл клиент на просмотр и готов забирать — жму быстрое
создание, и всё делается».

Разобрано, из чего состояла цепочка вручную: форма договора (стороны и объект
там уже подставлялись из сделки — это работало), сохранение, отдельный экран
графика начислений, отдельная форма задачи, редактирование объекта ради
статуса. Пять экранов, и любое звено терялось — договор есть, начислений нет,
объект висит свободным.

Сделано:

- **`deal-completion.ts`** — чистая логика: тип договора из типа сделки и типа
  объекта, статус объекта после сделки, номер вида «АР-2026-014», дата
  окончания (наём — 11 месяцев без дня, чтобы договор не подлежал
  государственной регистрации; управление — год), срок задачи. 18 тестов.
- **`completeDealAction`** — один Server Action на всю цепочку: договор →
  график начислений (переиспользует `buildPaymentSchedule`) → задача на
  подписание → статус объекта → `advanceDealStage` на этап «Договор».
  Обязателен только договор: PostgREST не даёт транзакции на пять таблиц, и
  ручная компенсация опаснее, чем частично оформленная сделка, которую видно
  в карточке. Поэтому ошибки необязательных шагов собираются в результат, а не
  роняют уже созданный договор.
- **`/deals/[id]/complete`** — один экран подтверждения с живым предпросмотром
  графика (тот же `buildPaymentSchedule`, что и на сервере: на экране и в базе
  одно и то же). Галочками снимается любой шаг. Если сторон или объекта нет —
  вместо мастера показывается, чего не хватает, со ссылкой в сделку.
  Существующий договор по сделке — предупреждение со ссылкой, не запрет.
- Кнопка «Оформить» в карточке сделки; на завершённой и отменённой не
  показывается. На карточке договора — строка «Создано: …» из параметра
  `?created=`.

Попутно выяснилось: `contract_number` в форме договора нет вообще — номера не
проставлялись никогда, в документах на их месте оставался пропуск. Мастер
теперь их генерирует.

`npm run check` зелёный, 230 тестов.
